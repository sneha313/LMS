...
if(isset($action) && isset($transactionid) && isset($empid))
{
	...
	/*if ($action=='BirthdayEmail')
	{
		//echo "testing birthday date for current date";
	
		$empquery=$db->query("select * from emp where empid='".$empid."'and birthdaydate=CURRDAY() and state='Active'");
		$emprow=$db->fetchAssoc($empquery);
		$empemail=$emprow['emp_emailid'];
		$manageremailid=$emprow['manager_emailid'];
		//to address
		$to=$empemail.",India.DataRND@ecitele.com"; 
		//subject of mail
		$sub="***Happy Birthday ".$emprow['empname']."***";
		$message.="<img src='cf_logo.png'/>";
		//body of mail
		$mailBody='
										DEAR strtoupper('.$firstname.'),<br><br>
										'.$message.'<br><br>
                                       ';
