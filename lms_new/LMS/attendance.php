		...
		<SCRIPT TYPE="text/javascript">
			...
			$('#AttInd').submit(function() {
			 ...
			 $.each( $( "#accordion h3"), function( i, val ) {
					...
					$(val).html("<table id='table-2'><tr><td width='30%'><b>"+$(val).text()+"</b></td><td width='70%'><table id='table-2'><tr><td>"+
					firstString+"</td><td>"+
					secondString+"</td><td>"+
					thirdString+'</td><td>'+
					fourthString+"</td><td>"+
				...
				<form id="AttInd" name="AttInd" method="post" action="attendance.php?AttInd=1">
					<fieldset>
						...
						<table id='table-2'>
							...
							{
							 echo '<tr>
								<td>Before 10 and After 4</td>
								<td>
									<select size="1" name="AdvCond1">
							...
							<input type="submit" class="submitBtn" value="Submit" name="AttInd">
							</td>
							</tr>
						</table>
					</fieldset>
...
function getDataForEMP($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
	...
	$empinfo="";
	$empinfo=$empinfo. '<tr><td width=200><u>';
	$counter=0;
	$count=0;
	while ($row = mysql_fetch_assoc($result)) {
		$in = $row["First"];
	...
	while ($w1 < date('Y-m-d',strtotime(getFriday($last)))) {
		...
		$subempinfo=$subempinfo. '<tr>
				...
				<th  class="both">WFH</th>
				<th  class="both">Total WFH</th>
	...
	if (getTotalHRDiff($tot,$dayCount) && $_SESSION['u_emplocation']!='MUM') {
		...
		$subempinfo=$subempinfo. '<tr><td colspan=1 align="right"><font color=red >Total Hr</font></td>';
		$subempinfo=$subempinfo. '<td ><font color=red>'.$tot.'</font></td><td></td>';
		$subempinfo=$subempinfo. '<td colspan=3 align="right"><font color=red >Total Hr with WFH</font></td>';
		$subempinfo=$subempinfo. '<td ><font color=red>'.$finalTotal.'</font></td><td></td></tr>';
		$totweekHrV=$totweekHrV+1;	
		$totFinalHrv=$totFinalHrv+1;
	} else {
		$subempinfo=$subempinfo. '<tr><td colspan=1 align="right">Total Hr</td>';
		$subempinfo=$subempinfo. '<td >'.$tot.'</td><td></td>';
		$subempinfo=$subempinfo. '<td colspan=3 align="right">Total Hr with WFH</td>';
		$subempinfo=$subempinfo. '<td >'.$finalTotal.'</td><td></td></tr>';
...
function getDataForEMP1($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
	...
	$subempinfoComp="";
	$empinfo="";
	$empinfo=$empinfo. '<tr><td width=200><u><span onclick="toggle(\'sub-'.$empID.'\');">';
	while ($row = mysql_fetch_assoc($result)) {
		$in = $row["First"];
		$out= $row["Last"];
		$dayHr1=timediffinHR($in,$out);
	...
	while ($w1 < date('Y-m-d',strtotime($last))) {
		...
		$subempinfo=$subempinfo. '<tr>
				...
				<th  class="both">Out</th>
				<th  class="both">Total Hr</th>
				<th  class="both">WFH Hr</th>
				<th  class="both">Total</th>
				<th  class="both">Type Of day</th>
			</tr>';
		$w1= date('Y-m-d', strtotime("$wkst 7 day"));
		$tmpDate='00:00:00';
...
function createTableHeader($P10to4, $hr9, $hr45) {
	...
	echo $hr9;
	echo '</th><th  class="both">No of Total Hr-';
	echo $hr45;
...
if (isset($_REQUEST['AttInd'])) {
			...
			echo "<h2><u><center>Access details from $fromDate to $toDate for ";
			
			if (isset($_REQUEST['getDeptemp'])) {
				echo getempName($_REQUEST['getDeptemp']);
			}
			if (isset($_REQUEST['UDept'])) {
				echo " (" . $_REQUEST['UDept'] . ")</center><u></h2><br>";
			}
