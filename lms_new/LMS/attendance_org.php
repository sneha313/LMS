		...
		<SCRIPT TYPE="text/javascript">
			...
			$('#AttInd').submit(function() {
			 ...
			 $.each( $( "#accordion h3"), function( i, val ) {
					...
					if ( first > 40 ) {
						var firstString = "<font>"+$(val).next().find(".teamMorning").text();
					}  else { 
						var firstString =$(val).next().find(".teamMorning").text();
					}
					if ( second > 40 ) { 
						var secondString = "<font>"+$(val).next().find(".teamEvening").text();
					}  else { 
						var secondString =$(val).next().find(".teamEvening").text();
					}
					if ( third > 40 ) { 
						var thirdString = "<font>"+$(val).next().find(".teamDaily").text();
					}  else { 
						var thirdString =$(val).next().find(".teamDaily").text();
					}
					if ( fourth > 40 ) { 
						var fourthString = "<font>"+$(val).next().find(".teamWeek").text();
					}  else { 
					...
					secondString+"</td><td>"+
					thirdString+'</td><td>'+
					fourthString+"</td></tr></table></td></tr></table>");
			});
...
function getTeamPercentage() {
	...
	global $sumOfWeeklyhrDays;
	global $sumOfTotalWeeklyhrDays;
	If($sumOfTotalInDays!=0 && $sumOfTotalOutDays!=0 && $sumOfTotalDailyhrDays!=0 && $sumOfTotalWeeklyhrDays!=0) {
		echo "<tr>
				<td><b>Total</b>
...
if (isset($_REQUEST['AttInd'])) {
			...
			echo "<h2><center>Access details from $fromDate to $toDate for ";
			
			if (isset($_REQUEST['getDeptemp'])) {
				echo getempName($_REQUEST['getDeptemp']);
			}
			if (isset($_REQUEST['UDept'])) {
				echo " (" . $_REQUEST['UDept'] . ")</center></h2><br>";
			}
