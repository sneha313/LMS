			...
			if ((getTotalLeaves($employeeId)) < -5) {
				...
				echo 'if($("#'.$divId.'").is(":visible")) {
							...
							if(diffDays > numOfDays) {
								BoostrapDialog.alert("Yo cant apply leave more than "+numOfDays+"");
								return false;  
							} else { 
							    	$.ajax({ 
										data: $(this).serialize(), 
	...
	function getDisplayDatesJs($section) {
		...
		echo '$("#displayDates").submit(function() {
			...
			if(totaloptionalleaves> 2) {
				BootstrapDialog.alert("You have applied 2 optional leaves already. Please don\'t apply optional leave again");
				return false;
			}
	...
	function getLeaveForm($page, $formName, $formId, $leaveTypeId, $hideSplLeaveId, $specialLeaveId, $fromDateId, $toDateId, $functionName) {
		...
		echo "<div class='form-group'>
				<div class='row'>
					<div class='col-sm-2'></div>
					<div class='col-sm-3 empid'><label for='empid'>Employee Id:</label></div>
	         		<div class='col-sm-5 empid'><input type='text' class='form-control' id='empid' readonly name='empid' value= " . $_SESSION['u_empid'] . " /></div>
	        		<div class='col-sm-2'></div>
	         	</div></div>";
		}
		echo "
		...
		<div class='row'>
		<div class='col-sm-2'></div>
		<div class='col-sm-3'><label for='fromDate'>From Date:</label></div>
		<div class='col-sm-5'>
			<div class='input-group'>
				<input type='text' id='$fromDateId' class='form-control open-datetimepicker' name='$fromDateId' readonly='true' required>
		...
		<div class='row'>
		<div class='col-sm-2'></div>
		<div class='col-sm-3'><label for='toDate'>To Date:	</label></div>
		<div class='col-sm-5'>
			<div class='input-group'>
				<input type='text' id='$toDateId' class='form-control open-datetimepicker1' name='$toDateId' readonly='true' required>
				<label class='input-group-addon btn'>
		...
		echo "<div class='form-group'>
				<div class='row'>
					<div class='col-sm-2'></div>
					<div class='col-sm-3'><label for='Leave_type'>Leave Type:</label></div>
					<div class='col-sm-5'><select class='form-control' id='$leaveTypeId'  name='leavetype' onchange='$functionName(document.$formName.leavetype.options[document.$formName.leavetype.selectedIndex].value);'>";
		        	...
		        	<div class='col-sm-3' ><label for='Special_leave'>Special Leave:</label></div>
		        	<div class='col-sm-5'>
		        	<SELECT class='form-control' id= '$specialLeaveId' name='specialLeaveType' disabled='disabled'>$options</SELECT>
		        	</div>
		        	<div class='col-sm-2'></div>
		        	</div>
		        	</div></div>
					...
					<div class='row'>
					<div class='col-sm-2'></div>
		        	<div class='col-sm-3'><label for='reason'>Reason:</label></div>
		        	<div class='col-sm-5'><textarea id='reason' class='form-control required' name='reason'></textarea></div>
		        	<div class='col-sm-2'></div>
		        	</div></div>
		        	<div class='form-group'>
					 <div class='row'>
									...
									if(preg_match('/(.*)(\[.*)/', $Day[$i],$splMatch)) {
												$mailBody=$mailBody."<div class='form-group'>
													<div class='row'>
														...
														$mailBody=$mailBody."<div class='col-sm-4'>
																<input type='text' class='fomr-control' readonly='true' name='Day" . $i . "Date' size='50' value=\"" . str_replace('[special]','',$splMatch[2]) . "\">
														</div>
													</div>
												</div>";
											}
