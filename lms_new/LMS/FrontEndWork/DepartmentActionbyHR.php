			...
			$("#add-form").on("submit", function(event) {
				...
				$.post("DepartmentActionbyHR.php?add", $(this).serialize(), function(data) {
					if (data.includes("department already exist")) {
						...
						var obj = jQuery.parseJSON(data);
						$('#example tbody tr:last').after('<tr role="row"><td class="sorting_1" style="display:none;">' + obj.ID + '</td><td>' + obj.mainDept + '</td><td>' + obj.subDept + '</td><td>' + obj.deptLocation + '</td><td>' + obj.deptMailId + '</td><td><a data-id="row-' + obj.ID + '" href="javascript:editRow(' + obj.ID + ');" class="btn btn-success"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;<a href="javascript:removeRow(' + obj.ID + ');" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></a></td></tr>');
						$('#add-modal').modal('hide');
