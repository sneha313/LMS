...
<body>
	<center>
		<h3>Modify Employee Extra WFH Hour</h3>
	</center>
	<br>
	<?php
	function displaytable($transactionid) {
		...
		$sql=$db->query("select * from extrawfh where tid='".$transactionid."'");
		$row=$db->fetchassoc($sql);
		$childern=getChildren($_SESSION['u_empid']);
		$empnametresult=$db->query("select empname from emp where empid='".$row['eid']."' and state='Active'");
