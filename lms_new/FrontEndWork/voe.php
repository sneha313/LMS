	...
	<?php
		...
		echo '<div class="container-fluid">';
			...
			echo '<div class="col-sm-12">';// 12 column start
			    ...
			    $resultset1 = $db->pdoQuery($sqlquery1);
			    $countresultset1=$resultset1 -> count($sTable = 'empprofile', $sWhere = 'empid = "'.$employee_number.'"  and (address!="" or phonenumber!="" or fathername!="")' );
			    
			    if ($countresultset1!= 0) {
			    	//$row=$db->fetchArray($resultset1);
			    	$rows=$db->pdoQuery($sqlquery1)->results();
			    	foreach($rows as $row);
			    }
			   ...
			   if(isset($_REQUEST['profile']))
			   {
					...
					$sql = array('empid'=>$_REQUEST['employee_number'],'address'=>$_REQUEST['residentialAddress'],'phonenumber'=>$_REQUEST['phoneNo'],'fathername'=>$_REQUEST['father_name']);
					// use insert function
					$res = $db->insert('empprofile',$sql)->getLastInsertId();
				//	} elseif($db->countRows($resultset1)== 0) {
			   } elseif($countresultset1== 0) {
						...
						if(isset($_REQUEST['profile']))
						{
							...
							echo "</div>";
								...
								elseif (isset($_REQUEST['update'])) 
								{
									...
									//$res=$db->query($sql);
									$sql = array('address'=>$_REQUEST['residentialAddress'],'phonenumber'=>$_REQUEST['phoneNo'],'fathername'=>$_REQUEST['father_name']);
									// where condition array
									$sqlWhere = array('empid'=>$employee_number);
									// call update function
									$res = $db->update('empprofile', $sql, $sqlWhere)->affectedRows();
								} 
								//elseif ($db->countRows($resultset1)!=0  && ($row['address']!="" && $row['phonenumber']!="" && $row['fathername']!=""))
								elseif ($countresultset1!=0  && ($row['address']!="" && $row['phonenumber']!="" && $row['fathername']!="")) 
								{
									...
									// Insert voe form into database
										...
										{ 
											...
											$sql = array('employee_number'=>$_REQUEST['employee_number'],'vehicle_regno'=>$_REQUEST['vehicle_regno'],'vehicle_model'=>$_REQUEST['vehicle_model'],'vehicle_type'=>$_REQUEST['vehicle_type'],'nature_offuel'=>$_REQUEST['fuel_nature'],'original_cost'=>$_REQUEST['original_cost'],'claim_period'=>$_REQUEST['claim_period'],'distance_from_residenceto_office'=>$_REQUEST['distance_from_residenceto_office'],'vehicle_milage'=>$_REQUEST['vehicle_milage'],'fuelcost_perlitre'=>$_REQUEST['fuelcost_perlitre'],'total_fuelcost'=>$_REQUEST['total_fuelcost'],'repairs_maintence_expenses'=>$_REQUEST['repairs_maintence_expenses'],'wear_tear_cost'=>$_REQUEST['wear_tear_cost'],'driver_salary'=>$_REQUEST['driver_salary'],'total_voe'=>$_REQUEST['total_voe']);
											// use insert function
											$res = $db->insert('dynamic_table',$sql)->getLastInsertId();
											...
											for ($i = 1; $i <= 31; $i++)
											{
												if(isset($_REQUEST['dates'][$i]))
												{
													...
													//$res=$db->query($sql);
													$sql = array('employee_number'=>$_REQUEST['employee_number'],'claim_period'=>$_REQUEST['claim_period'],'vehicleused_dates'=>$_REQUEST['dates'][$i],'vehicleused_days'=>$_REQUEST['days'][$i]);
													// use insert function
													$res = $db->insert('vehicleuseddays',$sql)->getLastInsertId();
												}
												else
												{
													continue;
											...
											if (isset($_REQUEST['specific_date']))
											{
												for($j=0; $j<count($_REQUEST['specific_date']); $j++)
												{
													if (($_REQUEST['specific_date'][$j])!=0){	
														...
														//$res=$db->query($sql);
														$sql = array('employee_number'=>$_REQUEST['employee_number'],'claim_period'=>$_REQUEST['claim_period'],'vehicleused_dates'=>$date,'vehicleused_days'=>$day);
														// use insert function
														$res = $db->insert('specificdays',$sql)->getLastInsertId();
										...
										echo "<form name='formvoe' id='formvoe' method='POST' action='voe.php?insert=1'>";
											...
											<div class='form-group'>
												<div class='row'>
													...
													$rs = $db->pdoQuery($strSQL);
													$rsrow = $db->pdoQuery($strSQL)->results();
													// If  data about this employee is not presnt in dynamic table
													//if($db->countRows($rs) == 0)
													$rowcount=$rs -> count($sTable = 'dynamic_table', $sWhere = 'employee_number = "'.$employee_number.'"' );
													...
													if($rowcount == 0)
													{
														...
														//$rowcount=$sql -> count($sTable = 'dynamic_table', $sWhere = 'employee_number = "'.$employee_number.'"' );
													...
													if($rowcount!=0) {
														...
														//mysql_data_seek($rs, ($rowcount - 1));
													$results=$db->prepare($strSQL, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
														$results->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_ABS, ($rowcount - 1));
														//$data_seek=$db -> pdoQuery($rs,($rowcount - 1))->results();
														
														//while ($row = $db->fetchArray($rs))
														//while ($row = $rs->results())
