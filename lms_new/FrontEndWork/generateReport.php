			  ...
			  <form method="POST" action="" id="generateforHR" class="text-center">
			    <?php 
					if(strtoupper($_SESSION['user_dept'])=="HR"){?>
						<label class="radio-inline">
					      <input type="radio" name="HRreport" value="empid">Employee Name
					    </label>
					    <label class="radio-inline">
					      <input type="radio" name="HRreport" value="subdept">Sub Department
					    </label>
					    <label class="radio-inline">
					      <input type="radio" name="HRreport" value="Maindept">Main Department
					    </label>
					    <label class="radio-inline">
				...
				<div class="empid Selected" style="display:none;">
					...
					<div class="row">
						...
						<div class="col-sm-5">
								<input type="text" id="empname" class="form-control" name="empname">
						</div>
						<div class="col-sm-2"></div>
					</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-sm-5">
							<input type="text" id="employeeid" class="form-control" name="employeeid" value="">
						</div>
						<div class="col-sm-2"></div>
					</div>
					</div>-->
					...
					<div class="row">
						...
						<div class="col-sm-5">
							<select class="form-control" id="empMail" name="empMail" multiple="multiple">
								<?php
								$query="select distinct emp_emailid, empid from emp where state='Active'";
				...
				<div class="subdept Selected" style="display:none;">
					...
					<div class="row">
						...
						<div class="col-sm-5">
							...
							<?php 
							$queryrow="SELECT DISTINCT e.location FROM emp e, departments d where d.deptStatus='Active'";
					...
					<div class="row" id="maillistid" style="display: none;">
						...
						<div class="col-sm-5">
							<select class="form-control" id="subDeptMail" name="subDeptMail" multiple="multiple">
								<?php
								$queryrecord="select distinct emp_emailid, empid from emp where state='Active'";
				...
				<div class="Maindept Selected" style="display:none;">
					...
					<div class="row" id="mainDepartmentLoc">
						...
						<div class="col-sm-5">
							...
							<option value='Choose' selected>Choose Location</option>
							<?php 
							$queryloc="SELECT DISTINCT e.location FROM emp e, departments d where d.deptStatus='Active'";
								$sql = $db->pdoQuery($queryloc)->results();
								//for ($i=0;$i<$sql->rowCount();$i++)
								//$sql = $db->query("SELECT DISTINCT e.location FROM emp e, departments d where d.deptStatus='Active'");
								//for ($i=0;$i<$db->countRows($sql);$i++)
									foreach($sql as $result )
					...
					<div class="row" id="maindepartment" style="display:none;">
						...
						<div class="col-sm-5">
							...
							<?php 
							$querydept="SELECT DISTINCT mainDept FROM `departments` ";
								$query=$db->pdoQuery($querydept)->results();
					...
					<div class="row" id="mainDeptMail" style="display:none;">
						...
						<div class="col-sm-5">
							<select class="form-control" id="MaindeptmailList" name="MaindeptmailList" multiple="multiple">
								<?php
								$empquery="select distinct emp_emailid, empid from emp where state='Active'";
									$sql = $db->pdoQuery($empquery)->results();
									//for ($i=0;$i<$sql->rowCount();$i++)
									//$sql = $db->query("select distinct emp_emailid, empid from emp where state='Active'");
									//for ($i=0;$i<$db->countRows($sql);$i++)
										foreach($sql as $result)
				...
				<div class="allempreport Selected" style="display:none;">
					...
					<div class="row">
						...
						<div class="col-sm-5">
							<select class="form-control" id="locationforAllEmp" name="locationforAllEmp">
								<option value='Choose' selected>Choose Location</option>
							<?php 
							$querydis="SELECT DISTINCT e.location FROM emp e, departments d where d.deptStatus='Active'";
								$sql = $db->pdoQuery($querydis)->results();
								//for ($i=0;$i<$sql->rowCount();$i++)
								//$sql = $db->query("SELECT DISTINCT e.location FROM emp e, departments d where d.deptStatus='Active'");
								//for ($i=0;$i<$db->countRows($sql);$i++)
									foreach($sql as $result)
					...
					<div class="row" id="AllEmpMailList" style="display:none;">
						...
						<div class="col-sm-5">
							...
							<?php 
							$queryres="select emp_emailid, empid from emp where state='Active'";
