...
function getDay($empid,$date) {
	...
	$statusQuery="select * from `empleavetransactions` WHERE `transactionid` ='".$row['transactionid']."'";
        $statusRows=$db->pdoQuery($statusQuery)->results();
	...
	$getHoliday="select holidayname from ".$_SESSION['u_holidayListTable']." where date='".$date."'";
	...
	$holidayResult=$db->pdoQuery($getHoliday);
	$birthdayResult=$db->pdoQuery($getBirthday);
						
	if($holidayResult && $birthdayResult) {
		$holidayRow = $db->pdoQuery($getHoliday)->results();
		$birthdayRow = $db->pdoQuery($getBirthday)->results();
		$holidayRowcount=$holidayRow -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'" ' );
		
		if($holidayRowcount > 0 && isBirthday($empid,$date)) {
			return "Birthday and ".$holidayRow['holidayname']; 
		} elseif ($holidayRowcount > 0) {
...
function isBirthday($empid,$date) {
	...
	$birthdayResult=$db->pdoQuery($getBirthday);
	if($birthdayResult) {
		$holidayRowcount=$birthdayResult -> count($sTable = 'emp', $sWhere = 'empid = "'.$empid.'" and state="Active"' );

		$birthdayRows = $db->pdoQuery($getBirthday)->results();
		foreach($birthdayRows as $birthdayRow);
		if($holidayRowcount>0) {
...
function isHoliday($date) {
	global $db;
	#Check whether holiday is present on that day 
        $getHoliday="select holidayname from ".$_SESSION['u_holidayListTable']." where date='".$date."' and leavetype='Fixed'";
        $holidayResult=$db->pdoQuery($getHoliday);
        $holidayRowcount=$holidayResult -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'" and leavetype="Fixed"' );
        
        if($holidayResult && $holidayRowcount>0) {
		return 1;
	} else {
		return 0;
	}
...
function isOptionalHolidayApplied($date,$empid) {
	...
	$getHoliday="select holidayname from ".$_SESSION['u_holidayListTable']." where date='".$date."' and leavetype='Optional'";
	$holidayResult=$db->pdoQuery($getHoliday);
	$holidayResultRowcount=$holidayResult -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'" and leavetype="Optional"' );
	
	# check whether employee applied optional holiday
	$getOptionalHolidayquery="select * from empoptionalleavetaken where empid='".$empid."' and date='".$date."'";
	$getOptionalHolidayResult=$db->pdoQuery($getOptionalHolidayquery);
	$getOptionalHolidayResultRowcount=$getOptionalHolidayResult -> count($sTable = 'empoptionalleavetaken', $sWhere = 'empid = "'.$empid.'" and date="'.$date.'"' );
	 
	if($holidayResult && $holidayResultRowcount>0 && $getOptionalHolidayResultRowcount>0) {
		return 1;
	} else {
		return 0;
	}
...
function isPending($date,$empId) {
	...
	$pendingQuery="SELECT count(*) as count from `perdaytransactions` where `empid` ='".$empId."' AND `date`='".$date."' and status='Pending'";
	$pendingRows=$db->pdoQuery($pendingQuery)->results();
...
function isOptionalHoliday($date) {
        ...
        $getHoliday="select holidayname from ".$_SESSION['u_holidayListTable']." where date='".$date."' and leavetype='Optional'";
        $holidayResult=$db->pdoQuery($getHoliday);
        $holidayResultcount=$holidayResult -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'" and leavetype = "Optional"' );
        
        if($holidayResult && $holidayResultcount>0) {
                return 1;
        } else {
                return 0;
        }
...
function isFullDayPTO($date,$empId) {
     ...
     //   $getHoliday="select leavetype from perdaytransactions where date='".$date."' and empid='".$empId."'";
        $holidayResult=$db->pdoQuery($getHoliday);
       // $holidayResultcount=$holidayResult -> count($sTable = 'empleavetransactions', $sWhere = 'empid = "'.$_SESSION['u_empid'].'" and approvalstatus = "Pending"' );
        $holidayResultcountquery="SELECT COUNT(*) AS numrows FROM perdaytransactions a empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
        $holidayResultcount=$db->showQuery($holidayResultcountquery);
        if($holidayResult && $holidayResultcount>0) {
		$PTORows = $db->pdoQuery($getHoliday)->results();
		foreach($PTORows as $PTORow);
...
function isHalfDayPTO($date,$empId) {
      ...
      //  $getHalfDayPTO="select leavetype from perdaytransactions where date='".$date."' and empid='".$empId."'";
        ...
        $HalfDayPTOResult=$db->pdoQuery($getHalfDayPTO);
       $HalfDayPTOResultcountquery="SELECT COUNT(*) AS numrows FROM perdaytransactions a empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
        $HalfDayPTOResultcount=$db->showQuery($HalfDayPTOResultcountquery);
         
        if($HalfDayPTOResult && $HalfDayPTOResultcount>0) {
		$PTORows = $db->pdoQuery($getHalfDayPTO)->results();
		foreach($PTORows as $PTORow);
...
function isHalfdayWFH($date,$empId) {
        ...
        $WFHResult=$db->pdoQuery($getWFH);
        $WFHResultcountquery="SELECT COUNT(*) AS numrows FROM perdaytransactions a empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
        $WFHResultResultcount=$db->showQuery($WFHResultcountquery);
         
        if($WFHResult && $WFHResultResultcount>0) {
                $WFHRows = $db->pdoQuery($getWFH)->results();
...
function isSpecialLeave($date,$empId) {
        ...
        $SplLeaveResult=$db->pdoQuery($getSpecialLeave);
        $SplLeaveResultcountquery="SELECT COUNT(*) AS numrows FROM perdaytransactions a empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
        $SplLeaveResultcount=$db->showQuery($SplLeaveResultcountquery);
        
        if($SplLeaveResult && $SplLeaveResultcount>0) {
                $SPLRows = $db->pdoQuery($getSpecialLeave)->results();
                foreach($SPLRows as $SPLRow);
...
function isWFH($date,$empId) {
   ...
   //     $getWFH="select leavetype from perdaytransactions where date='".$date."' and empid='".$empId."'";
        ...
        $WFHResult=$db->pdoQuery($getWFH);
        $WFHResultcountquery="SELECT COUNT(*) AS numrows FROM perdaytransactions a empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
        $WFHResultcount=$db->showQuery($WFHResultcountquery);
         
        if($WFHResult && $WFHResultcount>0) {
                $WFHRows = $db->pdoQuery($getWFH)->results();
                foreach($WFHRows as $WFHRow)
...
function getShiftforDay($leavetype,$empId,$date) {
        ...
        $shiftResult=$db->pdoQuery($getShift);
        $shiftResultcount=$shiftResult -> count($sTable = 'perdaytransactions', $sWhere = 'date = "'.$date.'" and empid = "'.$empId.'" and leavetype="'.$leavetype.'"' );
        
        if($shiftResult && $shiftResultcount>0) {
                $shiftRows = $db->pdoQuery($getShift)->results();
...
function getDataForEMP($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
	...
	while ($w1 < date('Y-m-d',strtotime(getFriday($last)))) {
		...
		$result=$db->pdoQuery($queryS);
		$tempwkst=$wkst;
		$wkst=$w1;
		$rowcount=$result -> count($sTable = 'inout', $sWhere = 'EmpID = "'.$empID.'" and Date >= "'.$wkst.'" and date<"'.$w1.'"' );
	...
	//		continue;
		...
		for ($j=0;$j<7;$j++) {
			if ($flag){
				$rows = $db->pdoQuery($queryS)->results();
				foreach($rows as $row);
			}
...
function getTeamUntrackedPercentage() {
	...
	if($untrackedLeaves!=0) {
		$countOfUntrackedLeaves=$untrackedLeaves;
	} else {
		$countOfUntrackedLeaves=0;
	}
	echo "<table class='table'>
			<tr>
				<td><b>Total Untracked Leaves</b>
				<td class='teamUntrackedLeaves'><b>".$countOfUntrackedLeaves."</b></td>
			</tr>
		</table>";	
}
...
function getInformationForApprovedLeaves($empId,$empName,$presentDay,$db) {
	...
	# If information is present in perdaytransaction table, then check whether employee applied for half day i.e count of leaves is 0.5
        ...
        $checkInPerDayTransacrionTableForHalfDayPTOApplied= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Approved' and count='0.5' and leavetype='HalfDay'");
       if($checkInPerDayTransacrionTableForHalfDayPTOApplied->rowCount()>0) {
	  # If the employee applied Half Day and also applied compff
	  $HalfDayAndCompff=0;
	  $checkInPerDayTransacrionTableForComppffApplied= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Approved' and count='0' and leavetype='CompOff Leave'");
	  if($checkInPerDayTransacrionTableForComppffApplied->rowCount()>0) {
		# Since the employee applied compoff and half day PTO on same day, update row in inout table 
		# with status as "Data Exists"
		$HalfDayAndCompff=1;
		
	  }	
	  if($HalfDayAndCompff=1) {
                $inoutStatus="Data Exists";
                $inoutPrint="Data Exists";
          } else {
                $inoutStatus="Half Day PTO not applied";
                $inoutPrint="Half Day PTO not applied (HalfDay)";
          }
          $checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
          if($checkInOutforDataExists->rowCount()>0) {
            // $updateQuery="update `inout` set `state`='".$inoutStatus."' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
             //$updateQueryResult=$db->pdoQuery($updateQuery);

          	// update array data
          	$updateQuery = array('state'=>$inoutStatus);
          	// two where condition array
          	$aWhere = array('EmpID'=>$empId, 'Date'=>$presentDay);
          	// call update function
          	$updateQueryResult = $p->update('inout', $updateQuery, $aWhere)->affectedRows();
             echo "For a date: $presentDay and empid: $empId and emp: $empName ---> $inoutPrint\n";
          } else {
	     # IF he applied only halfday and didnt applied anyother leave for remaining half day, then insert into inout table as 
	     # Data exists
             	//$insertQuery="insert into `inout`(EmpID,EmpName,Department,Type,Date,First,Last,added_hrname,state) values('".$empId."','".$empName."','".$empDept."','".$empDept."','".$presentDay."','-','-','trackAttendence script','".$inoutStatus."')";
             	//$insertResult=$db->pdoQuery($insertQuery);

          	$insertQuery = array('EmpID'=>$empId,'EmpName'=>$empName,'Department'=>$empDept,'Type'=>$empDept,'Date'=>$presentDay,'First'=>'-','Last'=>'-','added_hrname'=>'trackAttendence script','state'=>$inoutStatus);
          	// use insert function
          	$insertResult = $db->insert('inout',$insertQuery)->getLastInsertId();
             	echo "For a date: $presentDay and empid: $empId and emp: $empName ---> $inoutPrint\n";
	  }
       } else {
	# Check whether the approved leave is of type "WFH", if yes, then check whether the row is present in inout table
	$checkInPerDayTransacrionTableForHalfDayPTOApplied= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Approved' and leavetype='WFH' and (shift='FirstHalf' or shift='SecondHalf')");
        if($checkInPerDayTransacrionTableForHalfDayPTOApplied->rowCount()>0) {
	    # If the employee applied WFH and also applied compff
	    $HalfDayAndCompff=0;
            $checkInPerDayTransacrionTableForComppffApplied= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Approved' and count='0' and leavetype='CompOff Leave'");
            if($checkInPerDayTransacrionTableForComppffApplied->rowCount()>0) {
                # Since the employee applied compoff and half day PTO on same day, update row in inout table 
	    ...
	    if($HalfDayAndCompff=1) {
                ...
                $inoutStatus="Half Day PTO not applied";
                $inoutPrint="Half Day PTO not applied (WFH)";
            }
            $checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
            if($checkInOutforDataExists->rowCount()>0) {
               //$updateQuery="update `inout` set `state`='".$inoutStatus."' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
               //$updateQueryResult=$db->query($updateQuery);
            	...
            	$aWhere = array('EmpID'=>23,'Date'=>$presentDay);
            	// call update function
            	$updateQueryResult = $p->update('inout', $updateQuery, $aWhere)->affectedRows();
               echo "For a date: $presentDay and empid: $empId and emp: $empName ---> $inoutPrint\n";
            } else {
		...
		# 6. Other HR Special leaves
                ...
                if($checkInOutforDataExists->rowCount()>0) {
                	...
                	// where condition array
                	$deleteRowInOut = array('EmpID'=>$empId,'Date'=>$presentDay);
                	// call update function
                	$deleteRowInOutResult = $db->delete('inout', $deleteRowInOut)->affectedRows();
	...
	//while ($row = $result->results()) {
		...
		echo "**************************** For an Employee: $empName *******************************\n";	
        	...
        	while (strtotime($presentDay) <= strtotime($toDate)) {
				...
				if(isHoliday($presentDay)) {
					# Set inout table status to "Data Exists" if the day is Employee birthday
					$checkBirthday=$db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and date='".$presentDay."'");
                                        if($checkBirthday->rowCount()>0) {
                	                        //$updateQuery="update `inout` set `state`='Data Exists' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
                                              //  $updateQueryResult=$db->query($updateQuery);

                                        	// update array data
                                        	$updateQuery = array('state'=>'Data Exists');
                                        	// where condition array
                                        	$aWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
                                        	// call update function
                                        	$updateQueryResult = $p->update('inout', $updateQuery, $aWhere)->affectedRows();
			...
			# Get the all the inout days of employee where status != "No Data"
        	        $checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."' and state!='No Data'");
        	        if($checkInOutforDataExists->rowCount()>0) {
        	         	# If the day is his birthday, and he came to office, update inout with state as "Data exists"
        	         	if(isBirthday($empId,$presentDay)) {
					 # Update inout table, if no information is present in perdaytransaction table
                                         $checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
                                         if($checkInOutforDataExists->rowCount()>0) {
                                         	$row = $checkInOutforDataExists->results();
						# If the employee didnt came to office on his birthday, remove entry in inout table which 
						# is added by track attendance script
						 ...
						 # If the employee came to office on his birthday, then update status as "Data Exists" in
						 # inout table
        	         				//$updateQuery="update `inout` set `state`='Data Exists' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
        	      		   			//$updateQueryResult=$db->query($updateQuery);
                                        	 	...
                                        	 	$aWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
                                        	 	// call update function
                                        	 	$q = $p->update('inout', $updateQuery, $aWhere)->affectedRows();
        	         				echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Data Exists\n";
						}
					}
        	         		$presentDay = date ("Y-m-d", strtotime("+1 day", strtotime($presentDay)));
        	         	...
        	         	# Condition 1:  Check whether his total working hour for that day is less than 3:30 hr. 
        	         			...
        	         			# else check in perdaytransaction table for that day and for that employee 
						# the "status" is updated.
        	         	...
        	         	if(strtotime($dayHr1) < strtotime("03:30:00")) {
        	         		# Check in perdaytransaction table, whether the employee applied leave 
					# and whether the status is "Approved"
        	         		$checkInPerDayTransacrionTableForStatus= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status ='Approved'");
        	         		if($checkInPerDayTransacrionTableForStatus->rowCount()==0) {
        	         			# If there is no information in perdaytransaction table, then employee didnt 
						# applied leave in LMS. So, update
        	         			...
        	         			$updateInOutTableWithHalfDayNotApplied = array('state'=>'Half Day PTO not applied');
        	         			// where condition array
        	         			$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
        	         			// call update function
        	         			$updateQueryResult = $p->update('inout', $updateInOutTableWithHalfDayNotApplied, $updateWhere)->affectedRows();
        	         			 
        	         			echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Half day pto not applied as his total working hours is less than 3:30 hr\n";
        	         		}
					...
					# Condition 2:  Check whether his total working hour for that day is more than 15:00 hr.
        	         		...
        	         		//$updateQueryResult=$db->query($updateQuery);
        	         		$updateInOutTableWithSwipeIssues = array('state'=>'Swipe issues');
        	         		// where condition array
        	         		$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
        	         		// call update function
        	         		$updateQueryResult = $p->update('inout', $updateInOutTableWithSwipeIssues, $updateWhere)->affectedRows();
        	         		 
        	         		echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Swipe issues\n";
        	         	} else {
					$checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."' and state!='Data Exists'");
                                        if($checkInOutforDataExists->rowCount()>0) {
						...
						//$updateInOutTableWithSwipeIssues="update `inout` set `state`='Data Exists' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
                                        	...
                                        	$updateInOutTableWithSwipeIssues = array('state'=>'Data Exists');
                                        	// where condition array
                                        	$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
                                        	// call update function
                                        	$updateQueryResult = $p->update('inout', $updateInOutTableWithSwipeIssues, $updateWhere)->affectedRows();	
				...
				$checkInPerDayTransacrionTableForStatus= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Pending'");
				if($checkInPerDayTransacrionTableForStatus->rowCount()>0) {
					# If there is pending transaction in perdaytransaction table, then update inout table with
					# status as "Leave is pending"
					...
					//$updateQueryResult=$db->query($updateInOutTableWithHalfDayNotApplied);
					$updateInOutTableWithHalfDayNotApplied = array('state'=>'Leave is pending for approval');
					// where condition array
					$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
					// call update function
					$updateQueryResult = $p->update('inout', $updateInOutTableWithHalfDayNotApplied, $updateWhere)->affectedRows();
				...
				$checkInPerdayTransactionTableForStatus= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status='Approved'");
				if($checkInPerdayTransactionTableForStatus->rowCount()>0) {
					$flag=0;
					# Update inout table, if transacrion in perdaytransaction table is Approved, then delete the row
					# in inout table
					$checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
					if($checkInOutforDataExists->rowCount()>0) {
						$row = $checkInOutforDataExists->results();
							...
							$deleteRowInOutResult = $db->delete('inout', $deleteRowInOut)->affectedRows();
								  ...
								  // $updateQuery="update `inout` set `state`='Data Exists' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
								   //$updateQueryResult=$db->query($updateQuery);
									...
									$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
									// call update function
									$updateQueryResult = $p->update('inout', $updateQuery, $updateWhere)->affectedRows();
								   echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Data Exists\n";
								}
							} 
				...
				# Since there is no row in inout table which has status as != "No Data", then the status can be
				# "No Data" or there is no row in inout table

				# Check condition for inout data update
				$checkInOutforDataExist= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
				$dayHr1="00:00:00";
				if($checkInOutforDataExist->rowCount()>0) {

					$row=$checkInOutforDataExist->results();
                                	$in = $row["First"];
                                	$out= $row["Last"];
                                	$dayHr1=timediffinHR($in,$out);
				}
				# If the employee total time is between the limits, update the status as "Data Exists"
						...
						echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Data Exists\n";
        	         		...
        	         		# Condition 1: If the day is his birthday, and he came to office, update the status as "Data Exists"
       	 		         	if(isBirthday($empId,$presentDay)) {
						$checkBirthday=$db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and date='".$presentDay."'");
						if($checkBirthday->rowCount()>0) {
							//$updateQuery="update `inout` set `state`='Data Exists' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
							...
							echo "For a date: $presentDay and empid: $empId and emp: $empName ---> Data Exists\n";
        	         		...
        	         		# Condition 2:  Check if any information is present in perdaytransaction table 
					# with status = "Approved" for that day for that employee
        	         		...
        	         		if($db->countRows($checkInPerdayTransactionTableForStatus)==0) {
        	         			$checkInOutforDataExists= $db->pdoQuery("SELECT * FROM `inout` WHERE empid='".$empId."' and Date='".$presentDay."'");
						# Update inout table state as "No Data", if no information is present 
						# in perdaytransaction table
        	         			if($checkInOutforDataExists->rowCount()>0) {
						...
						# If no row is present in inout and no row is present in perdaytransaction table,
						# Then insert row in inout with state as "No Data"
        	         				...
        	         				$insertQuery = array('EmpID'=>$empId,'EmpName'=>$empName,'Department'=>$empDept,'Type'=>$empDept,'Date'=>$presentDay,'First'=>'-','Last'=>'-','added_hrname'=>'trackAttendence script','state'=>'No Data');
        	         				// use insert function
        	         				$insertResult = $db->insert('test',$insertQuery)->getLastInsertId();
        	         				echo "For a date: $presentDay and empid: $empId and emp: $empName ---> No data\n";
        	         			}
        	         		} else {
						getInformationForApprovedLeaves($empId,$empName,$presentDay,$db);
        	         		}
					# In that case, update inout table with state as "Leave is pending for approval"
        	         		$checkInPerdayTransactionTableForStatus= $db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE empid='".$empId."' and date='".$presentDay."' and status = 'Pending'");
        	         		if($checkInPerdayTransactionTableForStatus->rowCount() > 0) {
        	         			//$updateQuery="update `inout` set `state`='Leave is pending for approval' where `EmpID`='".$empId."' and `Date`='".$presentDay."'";
        	         			//$updateQueryResult=$db->query($updateQuery);

        	         			$updateQuery = array('state'=>'Leave is pending for approval');
        	         			// where condition array
        	         			$updateWhere = array('EmpID'=>$empId,'Date'=>$presentDay);
        	         			// call update function
        	         			$updateQueryResult = $p->update('inout', $updateQuery, $updateWhere)->affectedRows();
...
function trackAttendence($result,$fromDate, $toDate, $db) {
	...
	foreach($rows as $row){
		...
		### If Employee joined after $fromDate, then set fromDate to Employee joining date
                if (strtotime($empJoiningDate) > strtotime($fromDate)) {
			$inOutQuery="SELECT * FROM `inout` where `Date` between '$empJoiningDate' and '$toDate' and `EmpID`='$empId' and `state` != 'Data Exists' and `state`!='Leave is pending for approval'";
                } else { 
			$inOutQuery="SELECT * FROM `inout` where `Date` between '$fromDate' and '$toDate' and `EmpID`='$empId' and `state` != 'Data Exists' and `state`!='Leave is pending for approval'";
		}
	        $inOutResult=$db -> pdoQuery($inOutQuery);
	        $inOutRows=$db -> pdoQuery($inOutQuery)->results();
		$foundEntry=0;
		//while ($inOutRow=$inOutResult->results()) {
