...
function getDay($empid,$date) {
        ...
        $statusRow = $statusresult->results();
        ...
        if($holidayResult) {
        	//$holidayRow = $db->fetchArray($holidayResult);
                $holidayRow = $holidayResult->results();
...
function getDataForEMP($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
	...
	$totweekHr=0;
	$totweekHrV=0;
	$subempinfoComp='<table border="4" class="table table-bordered"><tr>';
	...
	while ($w1 < date('Y-m-d',strtotime(getFriday($last)))) {
		$subempinfo= '<td><table class="table table-bordered">';
...
function getDataForEMP1($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
	...
	while ($w1 < date('Y-m-d',strtotime($last))) {
		$subempinfo= '<td colspan="5"><table class="table table-bordered">';
		$subempinfo=$subempinfo. '<tr class="info">
				<th>Day</th>
				<th>In</th>
				<th>Out</th>
...
$Employees=$db->pdoQuery("select * from emp where (empid=? and state=?);",array(323856,'Active'));
//$Employees=$db->query("select * from emp where empid='323856' and state='Active'");
$managerMailBody="";
//while($res=$db->fetchArray($Employees)) {
while($res=$Employees->results()) {
	$empid=$res['empid'];
	//$result=$db->query("select manager_emailid from emp where empid='$empid' and state='Active'");
	//$row=$db->fetchAssoc($result);
	$result=$db->pdoQuery("select manager_emailid from emp where (empid=? and state=?);",array($empid,'Active'));
	$row=$result->results();
	...
	if ( strtolower($res['role'])=="manager") {
		...
		while($teamRow=$db->fetchArray($team)) {
			$managerMailBody.='<table class="table table-bordered">
                        <tr class="info">    
                                <th>Name</th>
