	...
	<body>
		...
		<?php
			if(isset($_REQUEST['approveview']))
			{
				...
				$sql1=$db->pdoQuery($join1)->results();
			...
			if(isset($_REQUEST['approve']))
			{
				$tid=$_REQUEST['tid'];
				$childern=getChildren($_SESSION['u_empid']);
				$join = array('status'=>'Approved');
				$aWhere = array('tid'=>$tid);
				$sql = $db->update('extrawfh', $join, $aWhere)->affectedRows();
				//$sql=$db->pdoQuery($join);
				if($_SESSION['roleofemp']=="hr") {
				...
				$sqlapproved=$db->pdoQuery($showapproved)->results();
			...
			if(isset($_REQUEST['comment']))
			{
				...
				$dataArray = array('status'=>'Cancelled','comments'=>$comment);
				// where condition array
				$aWhere = array('tid'=>$tid);
				// call update function
				$result = $db->update('extrawfh', $dataArray, $aWhere)->affectedRows();
				if($_SESSION['roleofemp']=="hr") {
				...
				$sqlnotapprove=$db->pdoQuery($showcancelled)->results();
				...
				if($sqlnotapprove)
				{
					...
					$cmd = '/usr/bin/php -f sendmail.php '.$tid.' '.$row['empid'].'  notApproveLeave >> /dev/null &';
					exec($cmd);
				}
			}
