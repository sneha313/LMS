...
echo '<html>
	...
	echo '<div class="panel panel-primary">
		...
		$distinctyear="SELECT DISTINCT YEAR(startdate) as year FROM empleavetransactions where empid='".$_SESSION['u_empid']."' order by year desc";
			$sql=$db->pdoQuery($distinctyear);
			$distinctYears=array(); 
			//$leaveCount=$db->countRows($sql);
			//$leaveCount=$sql->rowCount();
			$leaveCount=$sql -> count($sTable = 'empleavetransactions', $sWhere = 'empid = "'.$_SESSION['u_empid'].'"' );
		...
		//	for($i=0;$i<$leaveCount;$i++)
			//{
				...
				$rows=$db->pdoQuery($distinctyear)->results();
				foreach($rows as $row){
			...
			foreach ($distinctYears as $year) {
				echo "<div id='".$year."'>";
					...
					$empleavetrans="select * from empleavetransactions where empid='".$_SESSION['u_empid']."' and startDate between '".$year."-01-01' and '".$year."-12-31' and
					(approvalstatus='Approved' or approvalstatus='Cancelled' or approvalstatus='Deleted') order by startDate";
					$sql=$db->pdoQuery($empleavetrans);
					$sqlcount=$sql -> count($sTable = 'empleavetransactions', $sWhere = 'empid = "'.$_SESSION['u_empid'].'" and startDate between "'.$year.'-01-01" and "'.$year.'-12-31" and
					(approvalstatus="Approved" or approvalstatus="Cancelled" or approvalstatus="Deleted")' );
					$empleaveapprove="select * from empleavetransactions where empid='".$_SESSION['u_empid']."' and startDate between '".$year."-01-01' and '".$year."-12-31' and
					(approvalstatus='Approved') order by startDate";
					$onlyApprovedLeaves=$db->pdoQuery($empleaveapprove);
					$totalCount=0;
					$p=$onlyApprovedLeaves -> count($sTable = 'empleavetransactions', $sWhere = 'empid = "'.$_SESSION['u_empid'].'" and startDate between "'.$year.'-01-01" and "'.$year.'-12-31" and
					...
					for($j=0;$j<$p;$j++)
						//for($j=0;$j<$db->countRows($onlyApprovedLeaves);$j++)
						{
							...
							$row=$db->pdoQuery($empleaveapprove)->results();
						...
						echo "<table class='table table-hover table-bordered leavehistory'>
							...
							$rows=$db->pdoQuery($empleavetrans)->results();
						//for($i=0;$i<$db->countRows($sql);$i++)
							...
							foreach($rows as $row)
						{
							...
							$query1="select * from perdaytransactions where (transactionid='".$tid."');";
							//$sql1=$db->query("select * from perdaytransactions where transactionid='".$tid."'");
							$sql1=$db->pdoQuery($query1);
							$queryrow1=$db->pdoQuery($query1)->results();
							echo '<tr>
								<td colspan="6">
