...
function getEmpSelectionBox($supervisorID,$selectedEmployee)
{
	...
	echo'</select></div>
			...
			if($db->countRows($leavetype) > 0){
				for($i=0;$i<$db->countRows($leavetype);$i++)
				{
					$row=$db->fetchAssoc($leavetype);
					echo '<option value="'.$row['regularleaveid'].'">'.$row['regularleave'].'</option>';
				}
			}
			echo '</select></div>
				<div class="col-sm-2"></div>
...
function getNumberOfHolidays($start,$end){
	...
	$holidays=array();
	$count=0;
	$result = $db->query("select date from ".$_SESSION['u_holidayListTable']." where date >= "."'". $start ."'" ." AND date <= " . "'". $end. "'");
...
function jqGrid_GetData($query,$request) {
	...
	for ($index = 0; $index < $numFields; $index++) {
		...
		$header = $results->getColumnMeta($index);
...
### Get regular leaves from table
	$LeavesTypes=array();
	$leavetype=$db->query("select id, regularleave from regularleaves");
	if($db->countRows($leavetype) > 0){
		while($row=$db->fetchAssoc($leavetype)) {
			array_push($LeavesTypes,$row['regularleave']);
		}
	}
