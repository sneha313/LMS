			...
			<?php
				function empHistory($empid,$query){
						...
						foreach($sql as $row)
					{
						...
						$sql1=$db->pdoQuery("select * from perdaytransactions where (transactionid=?);",array($tid))->results();
						if($_REQUEST['leaveType']=="ALL")
						{
							echo '<tr>
									<td colspan="6">
				...
				if(isset($_REQUEST['empid']) && isset($_REQUEST['leaveType']) )
				{
					...
					if($_REQUEST['empid']!="All")
					{
						...
						$queryres="SELECT empname FROM `emp` WHERE (empid='".$_REQUEST['empid']."');";
						$result1=$db->pdoQuery($queryres);
						$rows1=$db->pdoQuery($queryres)->results();
						foreach($rows1 as $row1)
						$queryres3="SELECT balanceleaves,carryforwarded FROM `emptotalleaves` WHERE (empid='".$_REQUEST['empid']."');";
						$result3=$db->pdoQuery($queryres3);
						$rows3=$db->pdoQuery($queryres3)->results();
						...
						foreach ($emplist as $empid)
						{
							...
							$row3=$db->fetchAssoc($result3);*/
							$query1="SELECT empname FROM `emp` WHERE empid=".$empid;
							$result1=$db->pdoQuery($query1)->results();
							//$row1=$result1->results();
							foreach ($result1 as $row1)
							$query3="SELECT balanceleaves,carryforwarded FROM `emptotalleaves` WHERE empid=".$empid;
							$result3=$db->pdoQuery($query3)->results();
							//$row3=$result3->results();
