<?php
	...
	function getSelectBoxOptions($query,$value,$option,$id,$default)
	{
		...
		//while($row=$db->pdoQuery($query)->results())
			foreach($rows as $row)
		{
			$dept=$row[$option];
	...
	function displayEmployeeList() {
		...
		$arowcount=$rResult -> count($sTable = 'emp', $sWhere = 'state = "Active"' );
	...
	// AJAX FROM JQUERY
	if ( isset($_GET['ajax']) ) {
		//call displayEmployeeList function to show all the records into datatable
		displayEmployeeList(); 
	}
	...
	if ( isset($_GET['add']) && isset($_POST) ) {
		...
		if ( !empty($_POST['newempid']) && !empty($_POST['newempusername']) && !empty($_POST['newempname']) && !empty($_POST['joiningdate'])) {
			//to generate the id by last insert query
			$id = mysql_insert_id();
			//to check department already exist or not
			$query =$db->pdoQuery("SELECT distinct empid FROM $sTable where empid='".$_POST['newempid']."'");
			$rowcount=$query -> count($sTable = 'emp', $sWhere = 'empid = "'.$_POST['newempid'].'"' );
			...
			if($rowcount >0)
			{
				...
				$managerquery="select empid,empusername,empname,emp_emailid,state from emp where empid='".$_POST['managername']."' and state='Active'";
				$manager=$db->pdoQuery($managerquery);
				$managerrows=$db->pdoQuery($managerquery)->results();
				foreach($managerrows as $managerrow)
				$managerrowcount=$manager -> count($sTable = 'emp', $sWhere = 'empid = "'.$p['managername'].'" and state="Active"' );
				...
				$insertquery=array('empid'=>$_POST['newempid'],'empusername'=>$_POST['newempusername'],
						'empname'=>$_POST['newempname'],'joiningdate'=>$_POST['joiningdate'],
						'birthdaydate'=>$_POST['birthdate'],'dept'=>$_POST['dept'],'managerid'=>$managerrow['empid'],
						'managerusername'=>$managerrow['empusername'],'managername'=>$managerrow['empname'],
						'role'=>$_POST['newrole'],'managerlevel'=>$_POST['hideaddmanagerlevel'],
						'group'=>$_POST['newgroup'],'emp_emailid'=>$_POST['newemail'],
						'manager_emailid'=>$managerrow['emp_emailid'],'location'=>$_POST['location']);
				// use insert function
				$res = $db->insert('emp',$insertquery)->getLastInsertId();
			}
			$querytable=" SELECT * FROM $sTable where empid='".$_POST['newempid']."'";
			$query1 = $db->pdoQuery($querytable);
			$rows=$db->pdoQuery($querytable)->results();
			foreach($rows as $row)
			die(json_encode($row));
		}
	}
	...
	if ( isset($_GET['remove']) && 0 < intval($_GET['remove']) ) {
		...
		$setEmpState = array('state'=>'InActive');
		// two where condition array
		$aWhere = array($sIndexColumn=>intval($_GET['remove']));
		// call update function
		$result = $db->update('emp', $setEmpState, $aWhere)->affectedRows();
	}
	...
	if ( isset($_GET['edit']) && intval($_GET['edit'])>0) {
		...
		global $aColumns;
		global $sIndexColumn;
		$queryedit="select * from $sTable WHERE $sIndexColumn = '" . intval($_GET['edit'])."' ";
		$query=$db->pdoQuery($queryedit);
		//$row=$query->results();
		$rows=$db->pdoQuery($queryedit)->results();
		foreach($rows as $row)
		$dept=$row['dept'];
		$empusername=$row['empusername'];
		$manageremailid=$row['emp_emailid'];
		// SAVE DATA
		if ( isset($_POST) ) {
			$p = $_POST;
			foreach ( $p as &$val ) //$val = mysql_real_escape_string($val);
			//condition to check add form will not be empty then insert the row
			if ( !empty($p['editempid']) && !empty($p['editempusername']) && !empty($p['editempname']) && !empty($p['editgroup']) && !empty($p['editemail']) && !empty($p['editjoiningdate']) && !empty($p['dept']) && !empty($p['managername']) && !empty($p['editbirthdate'])){
				
					/*$result="UPDATE $sTable SET empusername = '" . $p['editempusername'] . "', empname = '" . $p['editempname'] . "',
					joiningdate='".$p['editjoiningdate']."', birthdaydate='".$p['editbirthdate']."', dept='".$p['dept']."',
					`managerid`='".$p['managername']."',`managerusername`='".$row['empusername']."',`managername`='".$row['empname']."',
					`role`='".$p['editrole']."',`managerlevel`='".$p['hideeditmanagerlevel']."',
					`group`='".$p['editgroup']."',`emp_emailid`='".$p['editemail']."',`manager_emailid`='".$row['emp_emailid']."',
					`location`='".$p['location']."' WHERE state='Active' and empid='".$p['editempid']."'and $sIndexColumn = " . intval($_GET['edit']);
						
				$res=$db->pdoQuery($result)->results();*/

				$dataArray = array('empusername'=>$p['editempusername'],
						'empname'=>$p['editempname'],'joiningdate'=>$p['editjoiningdate'],
						'birthdaydate'=>$p['editbirthdate'],'dept'=>$p['dept'],'managerid'=>$p['managername'],
						'managerusername'=>$row['managerusername'],'managername'=>$row['managername'],
						'role'=>$p['editrole'],'managerlevel'=>$p['hideeditmanagerlevel'],
						'group'=>$p['editgroup'],'emp_emailid'=>$p['editemail'],
						'manager_emailid'=>$row['manager_emailid'],'location'=>$p['location']);
				// two where condition array
				$aWhere = array('id'=> intval($_GET['edit']));
				// call update function
				$res = $db->update('emp', $dataArray, $aWhere)->affectedRows();
				
			}
		}
		// GET DATA
		$queryrecord=" SELECT * FROM $sTable WHERE $sIndexColumn = '" . intval($_GET['edit'])."'";
		  		...
		  		<div class='form-group'>
					<div class='row'>
						...
						<div class='col-sm-3'>
    	  				...
    	  				<div class='col-sm-5'>
                			<input type='text' class='form-control' name='editempid' id='editempid' size='20' readonly/>
    	  				</div>
    	  				<div class='col-sm-2'></div>
    	  			</div>
    	  		</div>
