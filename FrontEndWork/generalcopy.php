...
function getLeaveForm($page, $formName, $formId, $leaveTypeId, $hideSplLeaveId, $specialLeaveId, $fromDateId, $toDateId, $functionName) {
	...
	if(isset($_REQUEST['empuser'])) {
		$query="select empid from emp where (empname='".$_REQUEST['empuser']."' and state='Active');";
		$res=$db->pdoQuery($query);
		$empId=getValueFromQuery($res,"empid");
	...
	echo "<div class='form-group'>
						...
						<div class='col-sm-5'>
							<div class='input-group'>
								<input type='text' name='$fromDateId' id='$fromDateId' size='20' class='required datepicker form-control' readonly='true'/>
	...
	echo "<div class='form-group'>
						...
						<div class='col-sm-5'>
							<div class='input-group'>
								<input type='text' name='$toDateId' id='$toDateId' size='20' class='form-control datepicker1 required' readonly='true'/>
								<label class='input-group-addon btn' for='date'>
									<span class='fa fa-calendar'></span>
								</label>
							</div>
...
function getConfirmLeaveSection($page, $empid, $fromDateId, $toDateId) {
	...
	for ($i = 0; $i < $_POST['totalDays']; $i++) {
		if (!empty($_POST['Day' . $i])) {
			...
			//$perdayquery = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>"");
			//$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
			...
			if ($_POST['Day' . $i] == "FullDay") {
				...
				$perdayquerydata = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>$_POST[$dateIndex],'leavetype'=>$leaveTypeSelected,'shift'=>'','status'=>$approvalStatus,'count'=>$perDayTransactionCount);
				//$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
				...
				$perdayquerydata = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>$_POST[$dateIndex],'leavetype'=>$leaveTypeSelected,'shift'=>$_POST[$index],'status'=>$approvalStatus,'count'=>$perDayTransactionCount);
				//$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
			...
			//	$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
				...
				$perdayquerydata = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>$_POST[$dateIndex],'leavetype'=>$leaveTypeSelected,'shift'=>'firstHalf','status'=>$approvalStatus,'count'=>$perDayTransactionCount);		
				//$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
				...
				$perdayquerydata = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>$_POST[$dateIndex],'leavetype'=>$leaveTypeSelected,'shift'=>'secondHalf','status'=>$approvalStatus,'count'=>$perDayTransactionCount);
				//$perdayquery = $db->insert('perdaytransactions',$perdayquerydata)->getLastInsertId();
			...
			if (preg_match('/optional/i', $_POST['Day' . $i.'Date'],$match)) {
				if(preg_match('/(.*)(\(Optional\))/', $_POST['Day' . $i.'Date'],$splMatch)) {
					...
					$getOptionalLeaveResult=$db->pdoQuery($getOptionalLeaveCount)->results();
					//$getOptionalLeaveResult=$getOptionalLeaveCount->results();
					
					$optionalLeavenames=array();
					foreach($getOptionalLeaveResult as $optionalleaverow1){
					...
					if(!in_array($splMatch[1],$optionalLeavenames)) {
						$optionalLeaveDatequery="SELECT * FROM ".$_SESSION['u_holidayListTable']." WHERE `holidayname` LIKE '".mysql_real_escape_string($splMatch[1])."'  order by date desc";
						...
						$optionalLeaveDate=getValueFromQuery($db->pdoQuery($optionalLeaveDatequery),"date");
						...
						$insertOptionalLeave = array('empid'=>$empid,'date'=>$optionalLeaveDate,'leave'=>mysql_real_escape_string($splMatch[1]),'state'=>$approvalStatus);
						// use insert function
						$result= $db->insert('empoptionalleavetaken',$insertOptionalLeave)->getLastInsertId();
						$optionalLeaveName="$splMatch[1] (Optional)";
						$perDayTransactionCount=0;
						
						$perdayOptionalquery = array('transactionid'=>$transaction_id,'empid'=>$empid,'date'=>$optionalLeaveDate,'leavetype'=>mysql_real_escape_string($optionalLeaveName),'','status'=>$approvalStatus,'count'=>$perDayTransactionCount);
						// use insert function
						$optionalResult= $db->insert('perdaytransactions',$perdayOptionalquery)->getLastInsertId();
						...
						//$result = $db -> pdoQuery($insertOptionalLeave);
	...
	if ((getTotalLeaves($empid) - $totalLeaves) < -60) {
		...
		if ($totalLeaves != 0 || ($totalLeaves == 0 && $_POST['splType'] != "%2F") || ($totalLeaves == 0 && isset($WFH)) || ($totalLeaves == 0 && isset($OnSite)) || $totalLeaves == 0 && isset($optionalLeaveapplied) && $optionalLeaveapplied==1) {
			...
			$query =  array('transactionid'=>$transaction_id,'empid'=>$empid,'startdate'=>$_POST[$fromDateId],'enddate'=>$_POST[$toDateId],'count'=>$totalLeaves,'reason'=>$reason,'approvalstatus'=>$approvalStatus,'approvalcomments'=>$approvalcomments);
		...
		if (isset($_POST['splType']) && $_POST['splType'] != "%2F") {
			...
			$splleaveidquery="select specialleaveid, specialleave from specialleaves where specialleave like '" . urldecode($_POST['splType']) . "%'";
			$splleaveidrow = $db -> pdoQuery($splleaveidquery)->results();
				
			//$splleaveidrow = $db -> fetchAssoc($splleaveidresult);
			//$result = $db -> query("select * from  empsplleavetaken where empid=" . $empid);
			$query='select * from  empsplleavetaken where (empid = "'.$empid.'") ;';
			$result=$db->pdoQuery($query);
			$count=$result -> count($sTable = 'empsplleavetaken', $sWhere = 'empid = "'.$empid.'" ' );
			
			$row=$db->pdoQuery($query)->results();
			if ($count>0) {
				//$row = $db -> fetchAssoc($result);
				$spltaken = $row['splleavetaken'] . $splleaveidrow['specialleaveid'] . "$approvalSplLeaveStatus:";
				$query = array('splleavetaken'=>$spltaken);
				$where=array('empid'=>$empid);
				//$query = "UPDATE `empsplleavetaken` SET `splleavetaken` = '" . $spltaken . "' WHERE `empid` ='" . $empid . "'";
				//$res = $db -> query($query);
				$res = $db->update('empsplleavetaken', $query, $where)->affectedRows();
			} else {
				...
				$spltaken=$splleaveidrow['specialleaveid'] . "$approvalSplLeaveStatus:";
				$dataArray = array('empid'=>$empid,'splleavetaken'=> $spltaken);
				$res = $db->insert('empsplleavetaken',$dataArray)->getLastInsertId();
			}
		}
		...
		if ($totalLeaves != 0 || ($totalLeaves == 0 && isset($_POST['splType']) && $_POST['splType'] != "%2F") || ($totalLeaves == 0 && isset($WFH)) || ($totalLeaves == 0 && isset($OnSite)) || $totalLeaves == 0 && isset($optionalLeaveapplied) && $optionalLeaveapplied==1) {
			...
			if ($page == "applyteammemberleave.php") {
				...
				$getleavesquery="SELECT  `empid` , `count` FROM empleavetransactions WHERE transactionid ='" . $transaction_id . "'";
				$rows1=$db->pdoQuery($getleavesquery)->results();
				foreach($rows1 as $row1)
				//$row1 =  $db->pdoQuery('select empid , count FROM empleavetransactions where (transactionid = ?) ;',array($transaction_id))->results();
				...
				$dataArray = array('balanceleaves'=>$reducedleaves);
				$Wherecondition = array('empid'=>$empid);
				$reduceleavesquery = $db->update('emptotalleaves', $dataArray, $Wherecondition)->affectedRows();
			...
			if ($page == "hrapplyleaveforall.php") {
				...
				$dataArray = array('balanceleaves'=>$reducedleaves);
				$Where = array('empid'=>$empid);
				$reduceleavesquery = $db->update('emptotalleaves', $dataArray, $Where)->affectedRows();
				//$reduceleavesquery = $db -> query("UPDATE  `emptotalleaves` SET  `balanceleaves` =  '" . $reducedleaves . "' WHERE  `empid` ='" . $empid . "'");
...
function getDelSection($page, $transactionid, $empid, $role) {
	...
	if ($page == "modifyempapprovedleaves.php") {
		...
		//$leaveCount =getValueFromQuery("$db->pdoQuery('select count from empleavetransactions where (transactionid = ?) ;',array($transactionid))->results();","count");
		//Increase count in emptotalleaves
		$balanceLeaves = getValueFromQuery("select balanceleaves from emptotalleaves where empid='" . $empid . "'", "balanceleaves");
		//$balanceLeaves =getValueFromQuery("$db->pdoQuery('select balanceleaves from emptotalleaves where (empid = ?) ;',array($empid))->results();","balanceleaves");
		...
		$dataArray = array('balanceleaves'=>$updatedleaves);
		// where condition array
		$aWhere = array('empid'=>$empid);
		// call update function
		$updateleaves = $db->update('emptotalleaves', $dataArray, $aWhere)->affectedRows();
		$splStatus = "A";
		...
		//$row1=$resultopt->fetchAll(PDO::FETCH_ASSOC);
		$resultopt=$db->pdoQuery($optionalQuery)->results();
		foreach($resultopt as $row1)
		$currentYear=date("Y");
		...
		$optionalLeaveRes=$db->pdoQuery($optionalLeaveQuery);
		$optionalLeavecount=$optionalLeaveRes -> count($sTable = 'empoptionalleavetaken', $sWhere = 'empid = "'.$empid.'"and date between "'.$currentYear.'-01-01" and "'.$currentYear.'-12-31" and state="Approved"' );
		
		if($optionalLeavecount>0) {
			//while($optionalLeaveRow = $db->fetchAssoc($optionalLeaveResult)){
		$optionalLeaveResult=$db->pdoQuery($optionalLeaveQuery)->results();
		foreach($optionalLeaveResult as $optionalLeaveRow){
				$datesRange=getDatesFromRange($row1['startdate'],$row1['enddate']);
				if(in_array($optionalLeaveRow['date'],$datesRange)) {
					$Where = array('date'=>$optionalLeaveRow['date'], 'empid'=> $empid);
					...
					$optionalLeave = $db->delete('empoptionalleavetaken', $Where)->affectedRows();
				}
		}
		}
	...
	if ($page == "selfleavestatus.php") {
		...
		$optionalLeavequeryResult=$db->pdoQuery($optionalLeaveQuery);
		$p=$optionalLeavequeryResult -> count($sTable = 'empoptionalleavetaken', $sWhere = 'empid = "'.$empid.'" and date between "'.$currentYear.'-01-01" and "'.$currentYear.'-12-31" and state="Pending"' );
		
		$optionalLeaveResult=$db->pdoQuery($optionalLeaveQuery)->results();
		//$optionalLeaveResult = $db->pdoQuery($optionalLeaveQuery)->results();
		if($p>0) {
			//while($optionalLeaveRow = $optionalLeaveResult->fetch(PDO::FETCH_ASSOC)){
			foreach($optionalLeaveResult as $optionalLeaveRow){
				$datesRange=getDatesFromRange($row1['startdate'],$row1['enddate']);
				if(in_array($optionalLeaveRow['date'],$datesRange)) {
					//$delOptionalLeave="DELETE FROM `lms`.`empoptionalleavetaken` WHERE `empoptionalleavetaken`.`date` ='".$optionalLeaveRow['date']."' and empid='".$empid."'";
	...
	//$leavetyperow = $leavetypequery->fetchAll(PDO::FETCH_ASSOC);
	$leavetypequery="SELECT leavetype FROM  `perdaytransactions` WHERE transactionid ='" . $transactionid . "' AND leavetype !=  'FullDay' AND leavetype !=  'HalfDay' AND leavetype !=  'WFH'
											//AND leavetype !=  'First Half-HalfDay & second Half-WFH' AND leavetype !=  'First Half-WFH & Second Half-HalfDay' AND leavetype NOT LIKE '%Optional%'";
	$leavetyperows=$db->pdoQuery($leavetypequery)->results();
	foreach($leavetyperows as $leavetyperow)
	if ($leavetyperow) {
		//Get leave type id from special leaves
		$leave=$leavetyperow['leavetype'];
		$leavetypeidquery = "select specialleaveid from specialleaves where specialleave LIKE '" . $leavetyperow['leavetype'] . "%'";
		//$leavetypeidrow = $leavetypeidquery->fetchAll(PDO::FETCH_ASSOC);
		$leavetypeidrow=$db->pdoQuery($leavetypeidquery)->results();
		$splleavetaken = getValueFromQuery("select splleavetaken from  empsplleavetaken where empid='" . $empid . "'", "splleavetaken");
		//$splleavetaken = getValueFromQuery("$db->pdoQuery('select specialleaveid from specialleaves where (empid = ?) ;',array($empid))->results();", "splleavetaken");
	...
	if ($page == "modifyempapprovedleaves.php") {
		...
		if (preg_match('/CompOff Leave/', $row1['reason'])) {
			...
			//$getinoutCompOffRow=$getinoutCompOff->fetchAll(PDO::FETCH_ASSOC);
			$getinoutCompOffRow= $db->pdoQuery('select Date from inout where (compofftakenday = ? and empid = ?) ;',array($row1['startdate'],$row1['empid']))->results();
		...
		$dataArray = array('approvalstatus'=>'Deleted','approvalcomments'=>$txtMessage);
		$Where = array('transactionid'=>$transactionid);
		$result1 = $db->update('empleavetransactions', $dataArray, $Where)->affectedRows();
		...
		if ($result1) {
			...
			$Where = array('transactionid'=>$transactionid);
			$updatePerDayTransaction = $db->update('perdaytransactions', $dataArray, $Where)->affectedRows();
			...
			//$getTransactionIdInformationRow=$getTransactionIdInformationResult->fetchAll(PDO::FETCH_ASSOC);
			$getTransactionIdInformationResult="select * from  `empleavetransactions` where transactionid ='".$transactionid."'";
			$getTransactionIdInformationRow = $db->pdoQuery($getTransactionIdInformationResult)->results();
			# Get the empid, fromdate and todate from the transactionid and execute the script which updates the inout table.
	...
	if ($page == "selfleavestatus.php") {
		...
		if(isset($_REQUEST['compoffdate'])) {
			...
			$aWhere = array('empid'=>$_SESSION['u_empid'], 'compofftakenday'=>$_REQUEST['compoffdate']);
			// call update function
			$inouttable = $db->update('inout', $dataArray, $aWhere)->affectedRows();
			// update array data
			$dataArray = array('approvalstatus'=>'Deleted','approvalcomments'=>'Deleted By Employee ('.$_SESSION['u_fullname'].')');
			// two where condition array
			$aWhere = array('transactionid'=>$transactionid);
			// call update function
			$writeComments = $db->update('empleavetransactions', $dataArray, $aWhere)->affectedRows();
			// update array data
			$dataArray = array('status'=>'Deleted');
			// two where condition array
			$aWhere = array('transactionid'=>$transactionid);
			// call update function
			$updatePerDayTransaction = $db->update('tperdaytransactionsest', $dataArray, $aWhere)->affectedRows();
			...
			if($writeComments) {
				...
				$aWhere = array('transactionid'=>$transactionid);
				// call update function
				$q = $db->update('perdaytransactions', $dataArray, $aWhere)->affectedRows();
				//$updatePerDayTransaction = $db -> query("update `perdaytransactions` set status='Deleted' WHERE transactionid ='" . $transactionid . "'");
			}
		}
...
function getModifySection($page, $role) {
	...
	if (isset($_SESSION['splLeaveType']) && !empty($_SESSION['splLeaveType'])) {
		...
		//$sql = $db -> query($query);
		$query1='select dayspermitted from specialleaves where (specialleave = "'.$_SESSION['splLeaveType'].'") ;';
	...
	if ($splLeaveCount > $permittedLeaves) {
		...
		for ($k = 0; $k < sizeof($keys); $k++) {
			if (preg_match('/Day/', $keys[$k])) {
				...
				$dataArray = array('count'=>$perDayTransactionCount,'leavetype'=>$leaveTypeSelected,'shift'=>$shift);
				// where condition array
				$aWhere = array('transactionid'=>$trasactionId,'date'=>$daydate);
				// call update function
				$query = $db->update('perdaytransactions', $dataArray, $aWhere)->affectedRows();
		...
		if ($page == "modifyempapprovedleaves.php") {
			...
			//$countrow = $countquery->fetchAll(PDO::FETCH_ASSOC);
			$countrowquery='select count,empid from empleavetransactions where (transactionid = "'.$trasactionId.'") ;';
			$countrows =$db->pdoQuery($countrowquery)->results();
			...
			//$balanceleavesrow = $balanceleaves->fetchAll(PDO::FETCH_ASSOC);
			$balanceleavesquery='select balanceleaves from emptotalleaves where (empid = "'.$countrow['empid'].'") ;';
			$balanceleavesrows =$db->pdoQuery($balanceleavesquery)->results();	
			foreach($balanceleavesrows as $balanceleavesrow)
			...
			if (($newbalanceleaves + (getCarryForwardedLeaves($countrow['empid']))) < -6) {
				...
				//$getTransactionIdInformationRow=$getTransactionIdInformationResult->fetchAll(PDO::FETCH_ASSOC);
				$getTransactionIdInformationRow =$db->pdoQuery('select * from empleavetransactions where (transactionid = ?) ;',array($trasactionId))->results();
				# Get the empid, fromdate and todate from the transactionid and execute the script which updates the inout table.
				$cmd = '/usr/bin/php trackWeeklyAttendance.php --empId='.$getTransactionIdInformationRow['empid'].' --fromDate='.$getTransactionIdInformationRow['startdate'].' --toDate='.$getTransactionIdInformationRow['enddate'].' >> /dev/null &';
				exec($cmd);
				
				displaytable($_REQUEST['tid']);
				$dataArray = array('balanceleaves'=>$newbalanceleaves);
				// where condition array
				$aWhere = array('empid'=>$countrow['empid']);
				// call update function
				$updatebalanceleaves = $db->update('emptotalleaves', $dataArray, $aWhere)->affectedRows();
		...
		if ($page == "selfleavestatus.php") {
			...
			//$countquery = $db -> query("select count,empid from empleavetransactions where transactionid='" . $trasactionId . "'");
			//$countrow = $countquery->fetchAll(PDO::FETCH_ASSOC);
			$countrow= $db->pdoQuery('select count,empid from empleavetransactions where (transactionid = ?) ;',array($trasactionId))->results();
			//Get balance leaves from emptotalleaves
			//$balanceleaves = $db -> query("select balanceleaves from emptotalleaves where empid='" . $countrow['empid'] . "'");
			//$balanceleavesrow = $balanceleaves->fetchAll(PDO::FETCH_ASSOC);
			$balanceleavesrow= $db->pdoQuery('select balanceleaves from emptotalleaves where (empid = ?) ;',array($countrow['empid']))->results();
			...
			if (($newbalanceleaves + (getCarryForwardedLeaves($countrow['empid']))) < -6) {
				...
				//$sql = $db -> query($query);
				$dataArray = array('count'=>$leavecount);
				// where condition array
				$aWhere = array('transactionid'=>$trasactionId);
				// call update function
				$q = $db->update('empleavetransactions', $dataArray, $aWhere)->affectedRows();
				if ($count == $count1) {
...
function getSubmitSection($transactionid, $page, $id, $url, $i) {
	...
	$query1='select * from perdaytransactions where (transactionid = "'.$transactionid.'") ;';
	$query = $db->pdoQuery($query1);
	$p= $query -> count($sTable = 'perdaytransactions', $sWhere = 'transactionid = "'.$transactionid.'"');
	...
	echo '<table class="table table-hover">
				  ...
				  <tbody>';
	$perdaytransactionsquery="select * from perdaytransactions where (transactionid='" . $transactionid . "');";
	$perdaytransactionsres = $db -> pdoQuery($perdaytransactionsquery);
	//for ($i = 0; $i < sizeof($LeavesTypes); $i++) {
		$rows= $db->pdoQuery($perdaytransactionsquery)->results();
		foreach($rows as $row){
		$options = "";
		$shiftOptions = "";
