...
if(isset($_REQUEST['showleaves']))
{
		...
		//$isEmpPresent=$db->pdoQuery($isEmpPresentquery);
		//if($isEmpPresent->rowCount()>0) {
		...
		if($db->hasRows($isEmpPresent)) {
			...
			//$manager=$db->pdoQuery("select empid,empusername,empname,emp_emailid from emp where empid='".$_POST['managername']."' and state='Active'");
		//$managerrow=$manager->results();
	   ...
	   $insertemp="INSERT INTO`emp` (`empid`,`empusername`,`empname`,`joiningdate`,
		...
		//$dataArray = array('empid'=>$_POST['newempid'],'empusername'=>$_POST['newempusername'],'empname'=>$_POST['newempname'],'joiningdate'=>$_POST['joiningdate'],'birthdaydate'=>$_POST['birthdate'],'dept'=>$_POST['dept'],'managerid'=>$managerrow['empid'],'managerusername'=>$managerrow['empusername'],'managername'=>$managerrow['empname'],'role'=>$_POST['newrole'],'managerlevel'=>$_POST['hideaddmanagerlevel'],'group'=>$_POST['newgroup'],'emp_emailid'=>$_POST['newemail'],'manager_emailid'=>$managerrow['emp_emailid'],'location'=>$_POST['location']);
		// use insert function
		//$insertemp = $db->insert('emp',$dataArray)->getLastInsertId();
...
if(isset($_REQUEST['editempdetails']))
{
	...
	//$getEmpDetails=$db->pdoQuery("select * from emp where empname='".$_POST['empname']."' and state='Active'");
	//$row=$getEmpDetails->results();
	...
	<script>
		$(document).ready(function(){
			$('.open-datetimepicker2').datepicker({
				dateFormat: 'yy-mm-dd',
		       	minView : 2,
		       	autoclose: true     
			});
...
if(isset($_REQUEST['del_emp']))
{
		...
		$('#delempform').submit(function() {
  		BootstrapDialog.confirm('Delete Employee!', function(result){
		if (result)
		{
  			$.ajax({
  					data: $(this).serialize(),
  				 	...
  				 	success: function(response) {
  				 		if(response.trim() == 'success') {
	  				   		BootstrapDialog.alert('Employee is set to InActive state.', function(){
	  	  				   		$('#loadhrsection').html(response);
	  	  					});
  				  		}
	  				 	else{
...
{
	//$sql=$db->pdoQuery("SELECT empid from emp where empname='".$_POST['empusername']."' and state='Active'");
	//$row=$sql->results();
	...
	/*$dataArray = array('state'=>'InActive');
	// where condition array
	$aWhere = array('empid'=>$row['empid']);
	// call update function
	$setEmpState = $p->update('emp', $dataArray, $aWhere)->affectedRows();*/
