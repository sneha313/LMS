	...
	<body>
		<div class="col-sm-12">
				<?php
					...
					if($resultdept) {
						$resultrow= $db -> pdoQuery($querydept)->results();
				...
				<?php
					...
					function getDay($empid,$date) {
						...
						$statusresult=$db->pdoQuery($statusQuery);
						...
						if($holidayResult && $birthdayResult) {
							$holidayRows = $db->pdoQuery($getHoliday)->results();
							$birthdayRows = $db->pdoQuery($getBirthday)->results();
							foreach($holidayRows as $holidayRow)
							foreach($birthdayRows as $birthdayRow)
							$holidayRowcount=$holidayResult -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'"' );
			...
			#                        }    
					...
					function isBirthday($empid,$date) {
						...
						if($birthdayResult) {
							...
							$birthdayRowcount=$birthdayResult -> count($sTable = 'emp', $sWhere = 'empid = "'.$empid.'" and state = "Active"' );
				
							if($birthdayRowcount>0) {
					...
					function isHoliday($date) {
						...
						$holidayResult=$db->pdoQuery($getHoliday);
						$holidayResultcount=$holidayResult -> count($sTable = $_SESSION['u_holidayListTable'], $sWhere = 'date = "'.$date.'" and leavetype = "Fixed"' );
				...
				# check whether employee applied optional holiday
						$getOptionalHolidayResultcount=$db->pdoQuery("select * from empoptionalleavetaken where empid='".$empid."' and date='".$date."'");
						$getOptionalHolidayResultcount=$getOptionalHolidayResult -> count($sTable = 'empoptionalleavetaken', $sWhere = 'empid = "'.$empid.'" and date = "'.$date.'"' );
				if($holidayResult && $holidayResultcount>0 && $getOptionalHolidayResultcount>0) {
							return 1;
						} else {
							return 0;
						}
					...
					function isFullDayPTO($date,$empId) {
						...
						$holidayResult=$db->pdoQuery($getHoliday);
						$holidayResultcount="SELECT COUNT(*) AS numrows FROM perdaytransactions a,empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
						if($holidayResult && $holidayResultcount>0) {
					...
					function isHalfDayPTO($date,$empId) {
						...
						if($HalfDayPTOResult && $holidayResultcount>0) {
					...
					function isWFH($date,$empId) {
						...
						$WFHResultcount="SELECT COUNT(*) AS numrows FROM perdaytransactions a,empleavetransactions b WHERE a.`empid` ='".$empId."' AND a.`date`='".$date."' and b.approvalstatus='Approved' and a.transactionid=b.transactionid";
					...
					function getShiftforDay($leavetype,$empId,$date) {
						...
						$shiftResultcount=$shiftResult -> count($sTable = 'perdaytransactions', $sWhere = 'empid = "'.$empId.'" and date = "'.$date.'" and leavetype="'.$leavetype.'"' );
				if($shiftResult && $shiftResultcount>0) {
					...
					function getWFHhour($empid,$date){
						...
						$result=$db->pdoQuery($query);
						$resultcount=$result -> count($sTable = 'extrawfh', $sWhere = 'eid = "'.$empid.'" and date = "'.$date.'" and status="Approved"' );
					...
					function getDataForEMP($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
						...
						while ($w1 < date('Y-m-d',strtotime(getFriday($last)))) {
							$subempinfo= '<td id="emptable"><table class="table table-bordered table-hover" style="font-size: 14px;">';
							$subempinfo=$subempinfo. '<tr class="info">
								<th>Day</th>
								<th>In</th>
								<th>Out</th>
								<th>Total Office Hr</th>
								<th>WFH Hr</th>
								<th>Total Hr</th>
								<th width="10px">Type Of day</th>
							</tr>';
							$w1= date('Y-m-d', strtotime("$wkst 7 day"));
							$tmpDate='00:00:00';
							...
							$resultrowcount=$result -> count($sTable = 'inout', $sWhere = 'First != "'.$tmpDate.'" and Last != "'.$tmpDate.'" and EmpID="'.$empID.'" and Date >= "'.$wkst.'" and Date < "'.$w1.'"' );
							...
							if (getTotalHRDiff($tot,$dayCount) && $_SESSION['u_emplocation']!='MUM') {
								...
								$subempinfo=$subempinfo. '<tr><td colspan=3 align="center"><font color=red >Total Office Hr</font></td>';
								$subempinfo=$subempinfo. '<td><font color=red>'.$tot.'</font></td>';
								$subempinfo=$subempinfo. '<td align="center"><font color=red >Total Hr with WFH</font></td>';
								$subempinfo=$subempinfo. '<td><font color=red>'.$finalTotal.'</font></td></tr>';
								$totweekHrV=$totweekHrV+1;	
								$totFinalHrv=$totFinalHrv+1;
							} else {
								$subempinfo=$subempinfo. '<tr><td colspan=3 align="center">Total Office Hr</td>';
								$subempinfo=$subempinfo. '<td >'.$tot.'</td>';
								$subempinfo=$subempinfo. '<td align="center">Total Hr with WFH</td>';
					...
					function getDataForEMP1($empID,$empName,$first,$last,$P10to4,$hr9,$hr45) {
						...
						while ($w1 < date('Y-m-d',strtotime($last))) {
							...
							$tempwkst=$wkst;
							$wkst=$w1;
							$resultcount=$result -> count($sTable = 'inout', $sWhere = 'First != "'.$tmpDate.'" and Last != "'.$tmpDate.'" and EmpID="'.$empID.'" and Date >= "'.$wkst.'" and Date < "'.$w1.'"' );
					...
					function createTableHeader($P10to4, $hr9, $hr45) {
						echo '<table id= "table-2" class="table table-bordered">
								<tr class="info">
									<th>Name</th>
									<th>No of days';
						if ($P10to4 == "NOTMEET") {
					...
					if (isset($_REQUEST['AttInd'])) {
								...
								<div class='panel-heading text-center'>
									<strong style='font-size:20px;'>Access details from $fromDate to $toDate for ";
								...
								if (($_SESSION['user_dept'] == 'HR') || ($_SESSION['u_empid'] == "420064")) {
									if ($getDept == 'ALL') {
										...
										$query = "SELECT * FROM `emp` WHERE state='Active' and empid = '".$_REQUEST['getDeptemp']."' ORDER BY empname ASC";
									...
									if ($getDept == 'ALL') {
										...
										foreach($rows as $row){
											...
											$getEmployeesQuery="select * from emp where state='Active' and dept='".$row['dept']."'";
											$getEmployeesQueryResult=$db ->pdoQuery($getEmployeesQuery)->results();
											//while($getEmpRow= $getEmployeesQueryResult->results()) {
											foreach($getEmployeesQueryResult as $getEmpRow){
