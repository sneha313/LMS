...
if(isset($action) && isset($transactionid) && isset($empid))
{
	if ($action=='PendingLeave')
	{
		...
		$empquery=$db->pdoQuery("select * from emp where (empid=? and state=?');",array($empid,'Active'));
		$emprow=$empquery->results();
		...
		$daysquery=$db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE (transactionid=?);",array($transactionid));
		$reasonquery=$db->pdoQuery("SELECT * FROM `empleavetransactions` WHERE (transactionid=?);",array($transactionid));
		$reasonsrow=$reasonquery->results();
		...
		$phone_num=$db->pdoQuery("select phonenumber,address from empprofile where(empid=?);",array($empid));
                $phone_row=$phone_num->results();
		...
		while($daysrow=$daysquery->results()) {
	...
	if ($action=='ApproveLeave')
	{
		...
		if(isset($role) || isset($hrid)) {
			...
			if(isset($hrid)) {
				...
				$query=$db->pdoQuery("select empname from emp where (empid=? and state=?);",array($hrid,'Active'));
				$row=$query->results();
				$hrname=$row['empname'];
			}
		}
	...
	if ($action=='HRApproveLeave')
	{
		//$empquery=$db->query("select * from emp where empid='".$empid."' and state='Active'");
		//$emprow=$db->fetchAssoc($empquery);
		$empquery=$db->query("select * from emp where (empid=? and state=?);",array($empid,'Active'));
		$emprow=$empquery->results();
		//$daysquery=$db->query("SELECT * FROM `perdaytransactions` WHERE transactionid='".$transactionid."'");
		//$phone_num=$db->query("select phonenumber,address from empprofile where empid=".$empid);
		//$phone_row=$db->fetchAssoc($phone_num);
		...
		//$reasonquery=$db->query("SELECT * FROM `empleavetransactions` WHERE transactionid='".$transactionid."'");
		//$reasonsrow=$db->fetchAssoc($reasonquery);
		$reasonquery=$db->query("SELECT * FROM `empleavetransactions` WHERE (transactionid=?);",array($transactionid));
		$reasonsrow=$reasonquery->results();
		if(isset($role) || isset($hrid)) {
			$hrname="";
			if(isset($hrid)) {
				...
				//$row=$db->fetchAssoc($query);
				$query=$db->pdoQuery("select empname,emp_emailid from emp where (empid=? and state=?);",array($hrid,'Active'));
				$row=$query->results();
				$hrname=$row['empname'];
			}
		}
	...
	if ($action=='ApplyCompOff')
	{
		...
		for($j=0; $j< $transactionid->rowCount(); $j++)
		//for($j=0; $j<count($transactionid); $j++)
		{
			...
			$daysquery=$db->pdoQuery("SELECT * FROM `perdaytransactions` WHERE (transactionid=?);",array($transactionid[$j]));
			$daysrow=$daysquery->results();
	...
	if ($action=='PendingWFHhours')
	{
		...
		//$reasonsrow=$db->fetchAssoc($reasonquery);
		$reasonquery=$db->pdoQuery("SELECT * FROM `extrawfh` WHERE (tid=?);",array($transactionid));
		$reasonsrow=$reasonquery->results();
		$empemail=$emprow['emp_emailid'];
		$manageremailid=$emprow['manager_emailid'];
